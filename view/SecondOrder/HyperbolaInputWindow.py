...
class HyperbolaInputWindow(Window):
    ...
    def create_widgets(self):
        ...
        tk.Label(frame_point2, text="A/2:").grid(row=0, column=0, padx=5, pady=5)
        self.entry_a = tk.Entry(frame_point2)
        self.entry_a.grid(row=0, column=1, padx=5, pady=5)
        tk.Label(frame_point2, text="B/2:").grid(row=1, column=0, padx=5, pady=5)
    ...
    def return_data(self):
        try:
            ...
            self.destroy()
            self.controller.set_model_info((xc, yc), a, b, self.debug_mode)
        except ValueError:
            self.destroy()
