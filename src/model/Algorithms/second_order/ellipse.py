...
class EllipseBresenhamAlgorithm(Algorithm):
    def compute_points(self, center, a, b):
        ...
        # Region 1
        while dx < dy:
            dot_list.extend(self.get_ellipse_points(center, IntensityDot(x, y)))
            x += 1
            dx += two_b_squared
            if d1 < 0:
                d1 += dx + b_squared
    ...
    @staticmethod
    def get_ellipse_points(center: IntensityDot, dot: IntensityDot):
        # Get all four symmetric points
        points = [
            IntensityDot(center.x + dot.x, center.y + dot.y),
            IntensityDot(center.x - dot.x, center.y + dot.y),
            IntensityDot(center.x + dot.x, center.y - dot.y),
            IntensityDot(center.x - dot.x, center.y - dot.y),
        ]
